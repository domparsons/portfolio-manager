...
def create_transaction(
    ...
    asset = get_asset_by_id(data.asset_id, db)
    if asset is None:
        logger.error(f"Asset not found for asset id {data.asset_id}")
    ...
    if data.user_timezone not in pytz.all_timezones:
        logger.error(f"Invalid timezone: {data.user_timezone}")
        raise HTTPException(status_code=400, detail="Invalid timezone")

    utc_date = convert_to_utc(data.purchase_date, data.user_timezone)
    ...
    logger.info(
        f"Transaction created: asset_id={data.asset_id}, type={data.type}, "
        f"quantity={data.quantity}, total=${data.quantity * data.price:.2f}"
    )
    return portfolio
